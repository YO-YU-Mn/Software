    ...
    <div style={{ padding: 28, flex: 1, overflowY: "auto" }}>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 20 }}>
        <div style={{ display: "flex", gap: 10, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ background: G, color: "#fff", padding: "8px 16px", borderRadius: 8, fontSize: 13, fontWeight: 700 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${T.border}` }}>
              {["ID", "Name", "Email", "Course", "Dept", "GPA", "Status"].map(h => (
          ...
          <tbody>
            {filtered.map(s => (
              <tr
                key={s.id}
                style={{ borderBottom: `1px solid ${T.border}20`, cursor: "pointer", transition: "background 0.12s" }}
                onClick={() => setPanel(s)}
              ...
              >
                ...
                <td style={{ padding: "11px 12px", fontSize: 12 }}>
                  <span style={{ color: DEPT_COLORS[s.dept] || T.accent, fontWeight: 500 }}>{s.course}</span>
                </td>
                ...
                <td style={{ padding: "11px 12px" }}>
                  <span style={{ background: (STATUS_COLORS[s.status] || T.muted) + "20", color: STATUS_COLORS[s.status] || T.muted, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>
                    {s.status}
                  </span>
                </td>
              </tr>
