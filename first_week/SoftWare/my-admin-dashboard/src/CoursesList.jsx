...
export default function CoursesList({ courses, setCourses, enrollments }) {
  const [showAdd, setShowAdd] = useState(false);
  const [panel,   setPanel]   = useState(null);
  const [form,    setForm]    = useState({ name: "", instructor: "", credit: "", capacity: "" });

  // Compute live student counts from enrollments
  ...
  const handleAdd = () => {
    if (form.name && form.instructor && form.credit && form.capacity) {
      setCourses(prev => [...prev, {
        id: Date.now(), ...form,
        credit: +form.credit, capacity: +form.capacity,
        semester: "Fall", dept: "General",
      }]);
    ...
    <div style={{ padding: 28, flex: 1, overflowY: "auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
        ...
        <button
          ...
          style={{ background: G, color: "#fff", border: "none", padding: "9px 20px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13, boxShadow: `0 4px 14px ${T.accent}35` }}
        >+ Add Course</button>
      </div>
        ...
        {liveCourses.map(c => {
          ...
          return (
            <div
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = color + "55"; e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 12px 28px ${color}18`; }}
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                ...
                <span style={{ background: color + "20", color, padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600, flexShrink: 0 }}>{c.credit} cr.</span>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: T.muted, marginTop: 5 }}>
                <span>Click to manage capacity</span>
                <span style={{ color: pct >= 100 ? T.red : pct >= 80 ? T.yellow : T.green, fontWeight: 700 }}>{Math.min(pct, 100)}%</span>
              </div>
              {full && (
                <div style={{ background: T.red + "15", color: T.red, padding: "6px", borderRadius: 7, marginTop: 10, fontSize: 11, textAlign: "center", fontWeight: 700 }}>⚠ FULL</div>
              )}
            </div>
          );
        })}
