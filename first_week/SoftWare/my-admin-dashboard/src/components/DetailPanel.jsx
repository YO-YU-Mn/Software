...
export function DetailPanel({ item, type, onClose, onEditCapacity }) {
  ...
  const color = isCourse ? (DEPT_COLORS[item.dept] || T.accent) : (STATUS_COLORS[item.status] || T.muted);
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, backdropFilter:"blur(4px)" }} onClick={onClose}>
      <div style={{ background:T.card, border:`1px solid ${color}50`, borderRadius:20, width:430, maxWidth:"93vw", overflow:"hidden", boxShadow:`0 32px 80px rgba(0,0,0,0.6)` }} onClick={e=>e.stopPropagation()}>
        <div style={{ height:3, background:`linear-gradient(90deg,${color},${color}40)` }} />
        <div style={{ padding:"26px 26px 22px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
            <div>
              <div style={{ fontSize:10, color:T.muted, letterSpacing:2, textTransform:"uppercase", marginBottom:5 }}>{isCourse?"Course Details":"Student Profile"}</div>
              ...
              {!isCourse && <span style={{ marginTop:6, display:"inline-block", background:color+"22", color, padding:"2px 10px", borderRadius:20, fontSize:11, fontWeight:600 }}>{item.status}</span>}
            </div>
            <button onClick={onClose} style={{ background:T.surface, border:`1px solid ${T.border}`, color:T.muted, width:30, height:30, borderRadius:8, cursor:"pointer", fontSize:14 }}>✕</button>
          </div>

          <div style={{ borderRadius:10, overflow:"hidden", border:`1px solid ${T.border}` }}>
            {rows.map(([k,v],i) => (
              <div key={k} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", background:i%2===0?T.surface:T.card }}>
            ...
            {isCourse && (
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 14px", background:rows.length%2===0?T.surface:T.card }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {editCap ? (
                    <>
                      <input type="number" value={capVal} onChange={e=>setCapVal(e.target.value)} autoFocus
                    ...
                    <>
                      ...
                      <button onClick={()=>{setCapVal(String(item.capacity));setEditCap(true);}} style={{ background:T.accent+"22", color:T.accent, border:"none", borderRadius:5, padding:"2px 9px", cursor:"pointer", fontSize:11, fontWeight:600 }}>Edit</button>
                    </>
                  )}
                </div>
              </div>
          ...
          {isCourse && (
            <div style={{ marginTop:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:T.muted, marginBottom:6 }}>
                ...
                <span style={{ color:pct>=100?T.red:pct>=80?T.yellow:T.green, fontWeight:700 }}>{item.students}/{item.capacity} ({Math.min(pct,100)}%)</span>
              </div>
          ...
          {!isCourse && typeof item.gpa === "number" && (
            <div style={{ marginTop:16 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:T.muted, marginBottom:6 }}>
                <span>GPA</span>
                <span style={{ color:item.gpa>=3.7?T.green:item.gpa>=3.0?T.accent:T.yellow, fontWeight:700 }}>{item.gpa} / 4.0</span>
              </div>
              <div style={{ height:6, background:T.border, borderRadius:4 }}>
          ...
          <button onClick={onClose} style={{ marginTop:18, width:"100%", background:G, color:"#fff", border:"none", padding:11, borderRadius:9, cursor:"pointer", fontWeight:700, fontSize:14 }}>Close</button>
        </div>
      </div>
    </div>
  );
