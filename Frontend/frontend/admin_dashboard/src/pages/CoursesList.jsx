...
export function CoursesList({ setPage }) {
  ...
  const fetchAllCourses = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/courses/allcourses`, { headers: { Authorization: token } });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/courses/addCourse`, payload, { headers: { Authorization: token } });
  ...
  const handleEditCapacity = async (id, val) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${import.meta.env.VITE_API_URL}/courses/updateCourse/${id}`, { capacity: val }, { headers: { Authorization: token } });
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/bulkCourses/bulk-upload`, formData, {
