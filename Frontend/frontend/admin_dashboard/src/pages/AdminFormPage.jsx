...
export function AdminFormPage({ setPage }) {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/admins/allAdmins`, { 
  ...
  const handleAddAdmin = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/admins/addAdmin`, payload, { 
        headers: { Authorization: token } 
      });
  ...
  return (
    <div className="p-7 flex-1 overflow-y-auto" style={{ background: theme.bg }}>
      ...
      {tab === "addAdmin" && (
        <div className="card" style={{ background: theme.card, borderColor: theme.border, padding: 20 }}>
          ...
          <div className="flex gap-2">
            <button onClick={handleAddAdmin} className="btn" style={{ background: theme.green, color: theme.bg, padding: "8px 18px", fontWeight: 700 }}>Save Admin</button>
            <button onClick={() => setForm({ code: "", name: "", email: "", password: "" })} className="btn" style={{ background: theme.border, color: theme.text, padding: "8px 18px" }}>Clear</button>
          </div>
        </div>
      )}
          ...
          {admins.length === 0 ? (
            ...
            admins.map(admin => (
              <div 
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = theme.accent + "55"; e.currentTarget.style.transform = "translateY(-4px)"; }}
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="px-2 py-1 rounded-full text-xs font-semibold" style={{ background: `${theme.accent}20`, color: theme.accent }}>#{admin.code}</span>
                </div>
      ...
      <DetailPanel key={panel?._id} item={panel} type="admin" onClose={() => setPanel(null)} onRefresh={fetchAdmins} />
    </div>
  );
}